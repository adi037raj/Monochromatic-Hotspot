...
public class MergeOverlapCylinders {
	...
	static ArrayList<Patterns>  mergeOverlapNew(ArrayList<Patterns> pattern) {
		...
		for(int i=1;i<pattern.size();i++)
		{
			...
			for(int j=0;j<temp.size();j++)
			{
				...
				if(isOveralappedNew(pattern.get(i), temp.get(j)))
				{
					flag=0;
					break;
				}
	...
	static boolean isOveralappedNew(Patterns p,Patterns q)
	{
		...
		int minHeight = Math.min(p.height, q.height);

		double volume=(double)(Math.abs(start-end))/(double) minHeight;
